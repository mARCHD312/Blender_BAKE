...
def check_mtls_and_meshes():
    ...
    print(mtls_with_more_instances)
...
def delete_uv_maps_of_objects_that_starts_with_O():
    for obj in bpy.data.objects:
            if  obj.type == "MESH" and obj.name.startswith("O"):
                try:
                    ...
                    # for uv_layer in obj.data.uv_layers:
                    #     if uv_layer.name != used_layer:
                    a = obj.data.uv_layers.get(used_layer.name)
                    obj.data.uv_layers.remove(a)
                except Exception as e:
                    print("Exception with deleteing usused uw maps: ")
                    print(e)
    ...
    for obj in bpy.data.objects:
        if obj.type == "MESH" and obj.name.startswith("O"):
            for mat in obj.data.materials:
                for node in mat.node_tree.nodes:                
                    if node.name == "UV Map": # check this when have time
                            mat.node_tree.nodes.remove(node)
                    #     pass
                    # else:
                    #     counter = 0
    ...
    # # if object have less than two material slots and is mesh tye
        if len(obj.material_slots)<2:
            ...
            if  obj.type == "MESH":
                print("ONE MTL")
                print(obj.name)
    #             # if materisl name in materiales with more instances
